...
export async function ruliwebCollector(input: CollectorInput): Promise<NormalizedTrend[]> {
  try {
    ...
    // Hard, code-enforced site restriction — don't just trust the model's obedience to
    // "site:ruliweb.com" in the prompt. If grounding didn't actually return a ruliweb.com
    // page, we have no real evidence at all, regardless of what the model claims.
    const ruliwebUrls = [
      ...new Set(
        groundingChunks
          .map((c) => c.web?.uri)
          .filter((uri): uri is string => Boolean(uri) && uri!.includes('ruliweb')),
      ),
    ]
    if (ruliwebUrls.length === 0 || parsed.trends.length === 0) return []

    const resolved = await resolveAll(ruliwebUrls)
    const primaryUrl = resolved.get(ruliwebUrls[0]) ?? ruliwebUrls[0]
