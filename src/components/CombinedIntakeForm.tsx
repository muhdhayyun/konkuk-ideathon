...
import {
  ...
  EMOTIONAL_OUTCOMES,
  SAMPLE_BRIEF,
} from '../pages/client-form/constants'
...
// The single-page intake form shared by /ai-agent and /inquiry/recommend: button-first
// required fields (Client profile, Occasion, Recipient, Quantity, Budget, Emotional
// outcome, notes-via-suggestions) plus a kept-from-/recommend optional section
// (delivery date, priority chips). Owns all of its own composition state and only
// hands the caller a finished ClientBrief on submit — network/clarify orchestration
// stays with the page, since that's where the two pages diverge.
export default function CombinedIntakeForm({ onSubmit }: CombinedIntakeFormProps) {
  ...
  // Same ClientBrief values as before — SAMPLE_BRIEF doesn't cover every field on this
  // combined form (mode, wish text), so those get reasonable defaults so the form is
  ...
  const handleSubmit = () => {
    ...
    onSubmit({ ...brief, notes: [wish.trim(), extras].filter(Boolean).join(' ') })
  }
          ...
          <>
            <section className="mt-8 rounded-2xl bg-white p-8 shadow-sm">
              ...
              <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                ...
                <button type="button" onClick={toggleQuantityOther} className={selectableClass(quantityOther)}>
